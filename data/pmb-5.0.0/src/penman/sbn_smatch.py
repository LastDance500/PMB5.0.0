...
def create_arg_parser():
    ...
    parser.add_argument("-s2", '--sbn_file2', default="G:\github\PMB5.0.0\src\model\\NeuralBoxer\\result\\Neuralboxer_en_long.txt", type=str,
...
if __name__ == '__main__':
    ...
    if len(sbn_data) != len(sbn_data2):
        ...
        for i in range(len(sbn_data)):
            sbn1 = sbn_data[i].replace("<", "-").replace(">", "+").split("\t")[1].strip()
            try:
                penman1 = SBNGraph().from_string(sbn1, is_single_line=True).to_penman_string()
            except Exception as e:
                print(f"original - {i}: wrong format: {e}")
                num += 1
                continue

            sbn2 = sbn_data2[i].replace("<", "-").replace(">", "+").replace("?", " ?").strip()
            try:
                ...
                print(f"{i}: wrong format: {e}")

        print(f"average f1 smatch score: {average_f1/(len(sbn_data)-num)}")
