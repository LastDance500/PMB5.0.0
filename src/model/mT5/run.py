...
def create_arg_parser():
    ...
    parser.add_argument("-s", "--save1", required=False, type=str,
                        default=os.path.join(path, "src/model/mT5/result/mT5_nl_standard.txt"),
                        help="path to save the result")
    parser.add_argument("-s2", "--save2", required=False, type=str,
                        default=os.path.join(path, "src/model/mT5/result/mT5_nl_long.txt"),
...
def main():
    ...
    if test_long == "true":
        ...
        bart_classifier.train(train_dataloader2, test_dataloader, lr=0.0001, epoch_number=0)
        bart_classifier.evaluate(dev_dataloader, save_path1+".dev")
        bart_classifier.evaluate(test_dataloader, save_path1)

    bart_classifier.model.save_pretrained(os.path.join(path, f"models/T5_seq2seq/{lang}"))
